...
class Form
{
	...
	{
		//echo 'Form::login()';
		if ( ( isset( $_POST['login'] ) || isset( $_POST['email'] )) && ( isset( $_POST['password'] ) || isset( $_POST['card'] ) ) )
		{
			...
			$return = User::login( $login , $password, $col, $passcol );
		}
		else
		{
			$return = false;
	 ...
	 *
	 */
	public static function get( $table, $part, $action = false, $id = false, $intype = false )
	{
		...
		foreach ($fields as $key => $value) {
			...
			if ( $table == 'posts' && $key == 'type' )
			{
				...
				else
				{
					...
					$type = mb_substr($part, 0, -1);
					//var_dump($intype);
					$type = $intype ? $intype : mb_substr($part, 0, -1);
					if ( isset( $return[$key]['value'][$type] ) )
					{
						$return[$key]['value'][$type]['act'] = true;
					}
			...
			foreach ($files as $col => $file) {
				...
				if ( $file['error'] == UPLOAD_ERR_OK )
				{
					...
					if ( move_uploaded_file( $tmp_file, $up_file ) )
					{
						...
						{
							foreach ( array( 'min' => 300, 'mid' => 600, 'big' => 900 ) as $size => $value)
							{
								...
								self::imageResize( $size_file, $up_file, $value, $file['type'], ( 75 + $size / 40 ) );
							}
	...
	public static function post()
	{
		...
		if ( isset( $values['rubrics'] ) )
		{
			$rubrics = array();

			foreach ($values['rubrics'] as $key => $value) {
				if ( $value == 'on' )
				{
					array_push($rubrics, $key);
				}
			}
			$values['rubrics'] = implode(',', $rubrics);
		}
	...
	public static function input_options( $table, $key, $part )
	{
		...
		if( $key == 'rubrics' )
		{
			$tmpRubrics = Db::db_select('rubrics', null, null, 'id');
			// echo '<pre>';
			// print_r($tmpRubrics);
			// echo '<pre>';
			$rubrics = $tmpRubrics;
		...
		(
			...
			(
				...
				(
					'include'	=>	true,
					//'title'	=>	'Имя в адресной строке (en)'
					'title'	=>	false,
					'tag'	=>	'input',
					'type'	=>	'hidden'
