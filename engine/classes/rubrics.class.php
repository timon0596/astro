...
class Rubrics
{
	...
	/*public static function list()
	{
		if ( self::$_list == null )
		{
			$tmp = Db::db_select('rubrics',null,null,'id');
			self::$_list = array();
			if ( is_array( $tmp ) )
			{
	...
	public static function get_list()
	{
		if ( self::$_list == null )
		{
			$tmp = Db::db_select('rubrics',['id','>',0],null,'id');
			self::$_list = array();
			if ( is_array( $tmp ) )
			{
				foreach ($tmp as $key => $value)
	 ...
	 *
	 */
	public static function posts($rubric, $limit)
	{
		$tmp = Db::db_select('posts',"`rubrics` REGEXP '^(".$rubric."|.+,".$rubric.",.+|.+,".$rubric."|".$rubric.",.+)$'",null,'id');
		$list = array();
		if ( is_array( $tmp ) )
		{
			foreach ($tmp as $key => $value)
		...
		// {
		// 	echo $item['id'] . ' (' . $item['rubrics'] . ')' . PHP_EOL;
		// }
		// echo '</pre>';
